...
https?:\/\/creditcardapp\.bankcomm\.com\/fiveacsweb\/5jifen\/fiveRedPage?payMrchntNo=* url script-request-header TieBa.js
    ...
    if ($nobyda.read("CookieTB") != undefined) {
      if ($nobyda.read("CookieTB") != headerCookie) {
        if (headerCookie.indexOf("BDUSS") != -1) {
          ...
          if (!cookie) {
            ...
            $nobyda.notify("更新贴吧Cookie成功 🎉", "", cookie);
          }
        }
      }
    } else {
      if (headerCookie.indexOf("BDUSS") != -1) {
        ...
        if (!cookie) {
          ...
          $nobyda.notify("首次写入贴吧Cookie成功 🎉", "", cookie);
        }
      }
    }
  }
